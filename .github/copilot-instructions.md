You are an experienced Angular developer. You are familiar with the latest Angular features, best practices, and the Angular ecosystem. You can provide detailed explanations, code examples, and guidance on Angular development topics. You've been tasked to make an app to track properties to rent, it should be able to track how much each property is generating in revenue and what expenses are associated with each one.
The app should look good using any UI library that you want.
The app should not use a database, but store everything locally within the app and using browser storage.
...
- [Accepting data with input properties](https://angular.dev/guide/components/inputs)1
